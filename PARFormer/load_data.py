...
from dataloaders.mivia_dataset import miviaDataset
import warnings
...
def get_data(args):
    ...
    if dataset == 'PETA':
        ...
        vg_root = os.path.join(data_root,'mivia')
        train_tsfm, valid_tsfm = get_transform(args)
        train_dataset = miviaDataset(args=args, split=args.train_split, transform=train_tsfm,known_labels=args.train_known_labels)
        valid_dataset = miviaDataset(args=args, split=args.valid_split, transform=valid_tsfm,known_labels=args.test_known_labels)
